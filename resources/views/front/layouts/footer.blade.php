<footer class="pt-5 border-top border-light shadow bg-white">
    <div class="container border-bottom border-2">
        <div class="row py-3 justify-content-around">

            <div class="col-12 col-sm-12 col-md-8 col-lg-9 d-flex flex-column align-items-center">

                <div class="about-us w-100">
                    <h1 class="h5 fw-bolder w-100 p-0 m-0 mb-3 text-start">
                        سامانه آموزش مجازی امامت
                    </h1>
                    <p class="text-start mt-1 pb-0 mb-0">
                        لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است.
                        لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است.
                    </p>
                    <p class="text-start mt-2">
                        آدرس : یاسوج - خیابان پاسداران
                    </p>
                </div>

                <div class="links w-100">
                    <h1 class="h5 fw-bolder w-100 p-0 m-0 mb-2 mt-1 text-start">
                        تماس باما
                    </h1>
                    <nav class="nav px-0  flex-column">
                        <li class="nav-link ps-0" href="#">
                            <span class="text-dark fw-medium fs-55">پشتیبانی محتوا : </span>
                            <spna class="text-muted text-nowrap">شماره تماس : 09176623243</spna>
                            <span>-</span>
                            <spna class="text-muted text-nowrap">شماره دفتر : 07433440000</spna>
                            <span>-</span>
                            <spna class="text-muted text-nowrap">ایتا : @mr-najafi</spna>
                        </li>
                        <li class="nav-link ps-0" href="#">
                            <span class="text-dark fw-medium fs-55">پشتیبانی فنی : </span>
                            <spna class="text-muted text-nowrap">شماره تماس : 09176623243</spna>
                            <span>-</span>
                            <spna class="text-muted text-nowrap">ایتا : @mr-najafi</spna>
                        </li>
                    </nav>
                </div>

            </div>

            <div class="col-12 col-sm-12 col-md-4 col-lg-3 d-flex flex-column align-items-center">
                <h1 class="h5 fw-bolder w-100 pb-2 p-0 m-0 mb-3 text-center">
                    <span>مجوز ها</span>
                </h1>
                <div class="d-flex justify-content-center">
                    <img src="{{asset("assets/image/footer/L-1.png")}}" alt="">
                    <img src="{{asset("assets/image/footer/L-2.png")}}" alt="">
                </div>
            </div>

        </div>
    </div>
    <div class="container py-2">
        <p class="text-center mt-1">
            کلیه حقوق مادی و معنوی سایت متعلق به آموزش مجازی امامت می باشد.
        </p>
    </div>
</footer>
